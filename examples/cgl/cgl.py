...
class CGL:
    ...
    def __init__(self, x, nu, gamma, mu0, mu2, sigma):
        ...
        self.muc = self.mut + np.abs(self.h)*np.cos(np.angle(self.gamma)/2)/2
    ...
    def assemble_first_derivative_operator(self): 
        ...
        self.D = scipy.sparse.csc_array((data, (rows, cols)), \
    ...
    def assemble_second_derivative_operator(self): 
        ...
        self.DD = scipy.sparse.csc_array((data, (rows, cols)), \
                                         shape=(self.nx, self.nx))
    ...
    def assemble_input_operators(self, xas):
        if hasattr(xas, '__len__'):
            ...
            for k in range (len(xas)):
                B[:,k] = np.exp(-(self.x - xas[k])**2/(2*self.sigma**2))
        else:
            B = np.exp(-(self.x - xas)**2/(2*self.sigma**2)).reshape(-1,1)
        return B
    ...
    def assemble_output_operators(self, xas):
        if hasattr(xas,'__len__'):
            ...
            for k in range (len(xas)):
                C[:,k] = np.exp(-(self.x - xas[k])**2/(2*self.sigma**2))
        else:
            C = np.exp(-(self.x - xas)**2/(2*self.sigma**2)).reshape(-1,1)
            
        return self.M.T@C
    
    def compute_exact_eigenvalues(self, n):
        return np.asarray([self.mu0 - self.cu**2 - self.nu**2/(4*self.gamma) - \
                 (j + 1/2)*self.h for j in range (n)])
...
def matrix_data(tmp_path_factory, comm):
    rank = comm.Get_rank()
    N, s = 100, 10
    path = tmp_path_factory.mktemp("matrix_pytest_data")
...
def test_matrix_pytest_files_exist(comm, matrix_data):
    rank = comm.Get_rank()
    if rank == 0:
...
def test_matrix_pytest(comm, matrix_data):
    rank = comm.Get_rank()
    N, s = 100, 10
    ...
    for i in range(6, 10):
        Y = res4py.read_bv(comm, fnames[i], ((Nl, N), s))
        res4py.bv_add(-1.0, Y, actions[i - 6](X))
        assert Y.norm() < 1e-10
        Y.destroy()

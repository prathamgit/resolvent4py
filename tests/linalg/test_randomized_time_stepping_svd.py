...
@pytest.fixture(params=[
    pytest.param((10, 10, 2, 60, 7), marks=pytest.mark.local),       # local: small problem
    pytest.param((100, 100, 3, 300, 7), marks=pytest.mark.development), # dev: medium problem
    pytest.param((1000, 1000, 5, 600, 7), marks=pytest.mark.main),     # main: large problem
])
...
def test_randomized_time_stepping_svd(comm, rank_size, test_params, random_matrix, test_output_dir):
    ...
    U, S, V = res4py.linalg.randomized_time_stepping_svd(linop, mass_linop, linop.apply_mat, 
                                                omega, n_periods, n_timesteps, 
                                                n_rand, n_loops, n_svals)
    # if rank == 0:
    #     plt.figure()
    #     plt.plot(s.real, 'ko')

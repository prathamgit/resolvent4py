...
class InputOutputMatrices:
    def __init__(
        self, comm, compute_B, compute_C, compute_dB, compute_dC, D, F
    ):
        self.comm = comm
...
class HinfComponentSVD:
    def __init__(
        self, comm, path_factors, factor_sizes, frequencies, IOMats, alpha=1
    ):
        self.comm = comm
        ...
        for i in range(len(self.freqs)):
            ...
            self.R_ops.append(
                LowRankLinearOperator(
                    self.comm, self.U[-1], self.S[-1], self.V[-1]
                )
            )
            # dt = tlib.time() - t0
    ...
    def evaluate_cost(self, params):
        ...
        if Ln._name != "LowRankLinearOperator":
            for i in range(len(self.freqs)):
                R = self.R_ops[i]
                M, _ = _compute_woodbury_update(
                    self.comm, R, B, K, C, RB, RHTC
                )
                xi[i] += _compute_trace_product_new(
                    R, R, self.IOMats.DDHT, self.IOMats.FFHT
                )
                xi[i] -= 2.0 * _compute_trace_product_new(
                    R, M, self.IOMats.DDHT, self.IOMats.FFHT
                )
                xi[i] += _compute_trace_product_new(
                    M, M, self.IOMats.DDHT, self.IOMats.FFHT
                )
        else:
            F1 = Ln.U.duplicate()
            F2 = Ln.U.duplicate()
    ...
    def evaluate_gradient(self, params):
        ...
        for i in range(len(self.freqs)):
            ...
            xi = _compute_trace_product_new(
                R, R, self.IOMats.DDHT, self.IOMats.FFHT
            ).real
            xi -= (
                2.0
                * _compute_trace_product_new(
                    R, M, self.IOMats.DDHT, self.IOMats.FFHT
                ).real
            )
            xi += _compute_trace_product_new(
            ...
            LiCT = Linv.conj().T
            LiCTMat = PETSc.Mat().createDense(
                LiCT.shape, None, LiCT, MPI.COMM_SELF
            )
            ...
            grad_B_i = R.apply_hermitian_transpose_mat(
                F6B, grad_B_i
            )  # Used to be F3B instead of F6B
            ...
            grad_p += (
                2.0
                * val_i
                * self.IOMats.compute_dBdp(
                    p, grad_B_i, Bp1, Bp0, self.IOMats.compute_dB
                )
            )
            SMat.destroy()
            ...
            F2C = R.apply_hermitian_transpose_mat(
                F5C, F2C
            )  # Used to be F1C instead of F5C
            F3C = M.apply_hermitian_transpose_mat(
                F5C, F3C
            )  # Used to be F1C instead of F5C
            bv_add(-1.0, F3C, F2C)
            F6C = self.IOMats.FFHT.apply_mat(
                F3C, F6C
            )  # Used to be F3C instead of F6C
            ...
            grad_s += (
                ...
                * val_i
                * self.IOMats.compute_dBdp(
                    s, grad_C_i, Cp1, Cp0, self.IOMats.compute_dC
                )
            )
            SMat.destroy()
...
class H2ComponentSVD:
    def __init__(
        self, comm, path_factors, factor_sizes, frequencies, weights, IOMats
    ):
        self.comm = comm
        self.freqs = frequencies
    ...
    def evaluate_cost(self, params):
        ...
        if Ln._name != "LowRankLinearOperator":
            for i in range(len(self.freqs)):
                ...
                J += w * _compute_trace_product_new(
                    R, R, self.IOMats.DDHT, self.IOMats.FFHT
                )
                J -= (
                    2.0
    ...
    def evaluate_gradient(self, params):
        ...
        dBdps = self.IOMats.compute_dBdp_lst(
            p, Bp1, Bp0, self.IOMats.compute_dB
        )
        dCdss = self.IOMats.compute_dBdp_lst(
            s, Cp1, Cp0, self.IOMats.compute_dC
        )
...
class H2Component:
    ...
    def evaluate_gradient(self, params):
        ...
        for i in range(len(self.freqs)):
            ...
            grad_Ki = (
                Idna - (K @ S @ CHTRB).conj().T
            ) @ grad_Ki_mat.getDenseArray()
            grad_K += 2 * w * grad_Ki
            grad_Ki_mat.destroy()
            SHTmat.destroy()
            ...
            for j in range(len(p)):
                ...
                Mj = (
                    K
                    @ S
                    @ self.CHTRBinterps[i][k + 1](p, s, dCHTRB, scals_dps)
                )
                ...
                bv_conj(dRB)
                grad_p[j] += (
                    2 * w * _compute_bv_contraction(self.comm, dRB, F5B)
                )
                Mjmat.destroy()
            Mat.destroy()
            ...
            for j in range(len(s)):
                ...
                scals_dps = np.zeros((ns, na))
                scals_dps[l,] = 1.0
                dCHTRB = self.CHTRBinterps[i][k + 1 + num_params_actu](
                    p, s, dCHTRB, scals_dps
                )
                ...
                bv_conj(dRHTC)
                grad_s[j] += (
                    2 * w * _compute_bv_contraction(self.comm, dRHTC, F5C)
                )
                Mjmat.destroy()
            KSmat.destroy()
...
class BodeFormula:
    ...
    def evaluate_cost(self, params):
        ...
        f = 1.0 / np.pi if np.min(self.freqs) > 0.0 else 1.0 / (2 * np.pi)
        for i in range(len(self.freqs) - 1):
            L = self.CHTRBinterps[i][0](p, s, CHTRB, scals_ps) @ K
            _, svals, _ = sp.linalg.svd(Id + L)
            integral += f * self.weights[i] * np.sum(np.log(1.0 / svals))
            kernel[i] = np.sum(np.log(1.0 / svals))
        L = self.CHTRBinterps[-1][0](p, s, CHTRB, scals_ps) @ K
        chi_gamma = (
            np.sum(self.open_loop_evals.real - self.gamma)
            - 0.5 * np.trace(1j * self.freqs[-1] * L).real
            - integral.real
        )
        exp_arg = self.alpha * np.abs(chi_gamma)
        value = (
            chi_gamma**2 * np.exp(exp_arg) if exp_arg < np.log(1e13) else 1e13
        )
        return value
    ...
    def evaluate_gradient(self, params):
        ...
        for i in range(len(self.freqs) - 1):
            ...
            for j in range(len(p)):
                ...
                grad_p[j] += (
                    f
                    * wi
                    * np.einsum("ij,ij", dCHTRB.conj(), (K @ S).conj().T)
                )
        ...
        for j in range(len(p)):
            ...
            grad_p[j] -= 0.5 * np.einsum(
                "ij,ij", dCHTRB.conj(), (1j * self.freqs[-1] * K).conj().T
            )
        ...
        for j in range(len(s)):
            ...
            scals_dps = np.zeros((ns, na))
            scals_dps[l,] = 1.0
            dCHTRB = self.CHTRBinterps[-1][k + 1 + num_params_actu](
                p, s, dCHTRB, scals_dps
            )
            grad_s[j] -= 0.5 * np.einsum(
                "ij,ij", dCHTRB.conj(), (1j * self.freqs[-1] * K).conj().T
            )
        ...
        if exp_arg < np.log(1e13):
            ...
            value = (
                ...
                if chi_gamma > 0
                else (2 * chi_gamma - self.alpha * chi_gamma**2) * expval
            )
        else:
            value = 1e13
...
class BodeFormulaSVD:
    ...
    def evaluate_cost(self, params):
        ...
        sumRe = (
            np.sum(self.open_loop_eigvals.real - self.sigma)
            - 0.5 * np.trace(1j * self.freqs[-1] * L).real
            - integral
        )
        objs = [RB, CRB, B, C]
    ...
    def evaluate_gradient(self, params):
        ...
        sumRe = (
            np.sum(self.open_loop_eigvals.real - self.sigma)
            - 0.5 * np.trace(1j * self.freqs[-1] * CRBa @ K).real
            - integral
        )
        ...
        if exp_arg < np.log(1e13):
            ...
            value = (
                ...
                if sumRe > 0
                else (2 * sumRe - self.alpha * sumRe**2) * expval
            )
        else:
            value = 1e13
...
class FOMStabilityComponent:
    def __init__(
        ...
        for i in range(len(shifts)):
            ...
            self.process_evals.append(lambda x: 1j * s - 1.0 / x)

    def evaluate_exponential(self, val):
        try:
            value = np.exp(self.alpha * (val.real + self.beta)).real
        except:
            value = 1e12
    ...
    def evaluate_cost(self, params, IOMats):
        ...
        for i in range(len(self.As)):
            linop = LowRankUpdatedLinearOperator(
                self.comm, self.As[i], B, K, C
            )
            D, E = eig(
                linop,
                linop.solve,
    ...
    def evaluate_gradient(self, params, IOMats):
        ...
        for i in range(len(self.As)):
            ...
            grad_p -= IOMats.compute_dBdp(
                p, grad_B, Bp1, Bp0, IOMats.compute_dB
            )
            M.destroy()
            # Grad with respect to s
            ...
            grad_s -= IOMats.compute_dBdp(
                s, grad_C, Cp1, Cp0, IOMats.compute_dC
            )
            M.destroy()
...
def create_objective_and_gradient(
    manifold, WhichGrads, H2Comp=None, StabComp=None, GainPen=None
):
    r"""
    ...
    def euclidean_gradient(*params):
        ...
        if StabComp != None:
            grad_pStb, grad_KStb, grad_sStb = StabComp.evaluate_gradient(
                params
            )

        ...
        if Ln._name != "LowRankLinearOperator":
            for i in range(len(self.freqs)):
                ...
                J += w * _compute_trace_product_new(
                    R, R, IOMats.DDHT, IOMats.FFHT
                )
                J -= (
                    2.0
                    * w
                    * _compute_trace_product_new(
                        R, M, IOMats.DDHT, IOMats.FFHT
                    )
                )
                J += w * _compute_trace_product_new(
                    M, M, IOMats.DDHT, IOMats.FFHT
                )
        else:
            F1 = Ln.U.duplicate()
            F2 = Ln.U.duplicate()
    ...
    def evaluate_gradient(self, params, IOMats):
        ...
        for i in range(len(self.freqs)):
            ...
            grad_p += (
                2.0
                * w
                * IOMats.compute_dBdp(p, grad_B_i, Bp1, Bp0, IOMats.compute_dB)
            )
            SMat.destroy()
            ...
            grad_s += (
                2.0
                * w
                * IOMats.compute_dBdp(s, grad_C_i, Cp1, Cp0, IOMats.compute_dC)
            )
            SMat.destroy()
...
def create_objective_and_gradient_new(
    manifold, IOMats, WhichGrads, H2Comp=None, StabComp=None
):
    r"""
    ...
    def cost(*params):
        ...
        JS = (
            StabComp.evaluate_cost(params, IOMats) if StabComp != None else 0.0
        )
        return JH2 + JS
    ...
    def euclidean_gradient(*params):
        ...
        if H2Comp != None:
            grad_pH2, grad_KH2, grad_sH2 = H2Comp.evaluate_gradient(
                params, IOMats
            )
        if StabComp != None:
...
def generate_initial_gains(params, IOMats, H2Comp, K):
    ...
    for i in range(len(H2Comp.freqs)):
        ...
        RHS += (
            2 * w * RHSma.reshape(-1).real
            if cc == 1
            else w * RHSma.reshape(-1)
        )
    ...
    Kvec = u[:, idces] @ np.diag(1.0 / s[idces]) @ (v[:, idces].conj().T @ RHS)

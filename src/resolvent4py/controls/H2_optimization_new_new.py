...
class HinfComponent:
    ...
    def evaluate_gradient(self, params):
        ...
        for i in range(len(self.freqs)):
            ...
            grad_p += (
                2.0 * val_i * self.IOMats.compute_grad_p(p, dBdps, grad_B_i)
            )
            SMat.destroy()
            ...
            grad_s += (
                2.0 * val_i * self.IOMats.compute_grad_p(s, dCdss, grad_C_i)
            )
            SMat.destroy()
...
class StabilityComponent:
    def __init__(
        ...
        for i in range(len(shifts)):
            ...
            self.process_evals.append(lambda x: shifts[i] - 1.0 / x)
            self.process_evals_adj.append(
                lambda x: shifts[i] - np.conj(1.0 / x)
            )
...
class StabilityComponentNew:
    def __init__(
        ...
        self.process_evals = [lambda x, s=s: s - 1.0 / x for s in shifts]
        self.process_evals_adj = [
            lambda x, s=s: s - np.conj(1.0 / x) for s in shifts
        ]
